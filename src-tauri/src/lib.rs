...
/// 统一处理 hrouter:// 与兼容的 ccswitch:// 深链接 URL
///
/// - 解析 URL
...
fn handle_deeplink_url(
    ...
    if !url_str.starts_with("hrouter://") && !url_str.starts_with("ccswitch://") {
        return false;
    }
...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.hrouter/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（输出到 <app_config_dir>/logs/hrouter.log）
            {
                ...
                log::info!("=== HRouter v{} started ===", env!("CARGO_PKG_VERSION"));
            }
            ...
            let db_path = app_config_dir.join(crate::config::APP_DATABASE_FILE_NAME);
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 HRouter 的工作方式。
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::get_codex_gui_status,
            commands::launch_codex_gui_installer,
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older HRouter version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including hrouter.db)\n\
            2) If you see “database version is newer”, please upgrade HRouter\n\
