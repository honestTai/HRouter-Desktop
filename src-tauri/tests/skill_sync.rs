...
fn symlink_dir(src: &std::path::Path, dest: &std::path::Path) -> bool {
    match std::os::windows::fs::symlink_dir(src, dest) {
        ...
        // Treat only that OS limitation as a skipped symlink-dependent test.
...
fn import_from_apps_does_not_rewrite_selected_app_directory() {
    ...
    let ssot_skill_dir = home.join(".hrouter").join("skills").join("codex-skill");
...
fn sync_to_app_removes_disabled_and_orphaned_ssot_symlinks() {
    ...
    let ssot_dir = home.join(".hrouter").join("skills");
    ...
    assert!(
        ...
        "second test symlink should be available after the first succeeds"
    );
...
fn uninstall_skill_creates_backup_before_removing_ssot() {
    ...
    let ssot_skill_dir = home.join(".hrouter").join("skills").join("backup-skill");
...
fn restore_skill_backup_restores_files_to_ssot_and_current_app() {
    ...
    let ssot_skill_dir = home.join(".hrouter").join("skills").join("restore-skill");
